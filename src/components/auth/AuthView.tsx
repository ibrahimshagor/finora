...
export const AuthView: React.FC = () => {
  ...
  const handleAutoRegister = async () => {
    ...
    if (!loginPassword || loginPassword.length < 6) {
      setErrorMessage('স্বয়ংক্রিয়ভাবে অ্যাকাউন্ট খুলতে পাসওয়ার্ড কমপক্ষে ৬ অক্ষরের হতে হবে।');
      return;
    }
    ...
    try {
      ...
      if (code === 'auth/email-already-in-use') {
        setErrorMessage('এই ইমেইল দিয়ে ইতোমধ্যে অ্যাকাউন্ট রয়েছে। পাসওয়ার্ড সঠিক কিনা পরীক্ষা করুন অথবা পাসওয়ার্ড রিসেট করুন।');
      } else {
        setErrorMessage(err.message || 'অ্যাকাউন্ট তৈরি করা সম্ভব হয়নি।');
      }
    } finally {
  ...
  const handleDirectGoogleEntry = async () => {
    const cleanEmail = loginEmail.trim();
    if (!cleanEmail) {
                ...
                <div className="mb-4 p-3.5 bg-rose-500/10 border border-rose-500/30 rounded-2xl text-xs text-rose-300 space-y-2.5 animate-in fade-in">
                  ...
                  {(errorDetailsCode === 'auth/invalid-credential' || 
                    ...
                    <div className="pt-2 border-t border-rose-500/20 space-y-2">
                      <p className="text-[11px] text-slate-300 font-semibold">নিচের যেকোনো একটি সহজ উপায়ে প্রবেশ করুন:</p>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {/* Auto-create account with the password user already typed */}
                        <button
                          type="button"
                          onClick={handleAutoRegister}
                          disabled={isSubmitting}
                          className="px-3 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-[11px] font-bold flex items-center justify-center gap-1.5 transition-all shadow-sm shadow-emerald-900/40 cursor-pointer text-center"
                          title="এই ইমেইল ও পাসওয়ার্ড দিয়ে সরাসরি অ্যাকাউন্ট খুলুন"
                        >
                          <Sparkles className="w-3.5 h-3.5 text-amber-300 shrink-0" />
                          <span>সরাসরি নতুন অ্যাকাউন্ট তৈরি করুন</span>
                        </button>

                        {/* Direct Google Entry */}
                        <button
                          type="button"
                          onClick={handleDirectGoogleEntry}
                          disabled={isSubmitting}
                          className="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-100 border border-slate-600 rounded-xl text-[11px] font-bold flex items-center justify-center gap-1.5 transition-colors cursor-pointer text-center"
                          title="Google অ্যাকাউন্ট দিয়ে সরাসরি সাইন ইন"
                        >
                      ...
                      <div className="flex items-center justify-between pt-1 text-[11px]">
                        <button
                          ...
                          className="text-emerald-400 hover:text-emerald-300 font-medium flex items-center gap-1 hover:underline cursor-pointer"
                        >
                          <Lock className="w-3 h-3" />
                          <span>পাসওয়ার্ড ভুলে গেছেন? রিসেট করুন</span>
                        </button>
                        <button
                          type="button"
                          onClick={handleSwitchToRegisterWithEmail}
                          className="text-slate-400 hover:text-slate-200 font-medium hover:underline cursor-pointer"
                        >
                          নিবন্ধন ফর্ম পূরণ করুন &rarr;
                        </button>
                      </div>
                    </div>
                  )}
